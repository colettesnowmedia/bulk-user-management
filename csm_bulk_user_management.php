...
class CSM_Bulk_User_Management {
    ...
    function add_menu() {
        ...
        add_users_page( __( "Bulk User Management", "csm_membership" ), __( "Bulk User Management", "csm_membership" ), "edit_users", "csm_bulk_user_management", array( $this, "admin_import" ) );
        add_options_page( __( "Bulk User Options", "csm_membership" ), __( "Bulk User Options", "csm_membership" ), "manage_options", "csm_users_opts", array( $this, "user_settings_page" ) );
        if (is_plugin_active('csm_membership/csm_membership.php')) {
            add_submenu_page( "css_members_admin", __("Bulk User Management", "csm_membership"), __( "Bulk User Management", "csm_membership" ), "edit_users", "csm_bulk_user_management", array( $this, "admin_import" ) );
        }
    }
    ...
    function user_settings_page() {
        ?>
            ...
            <h1><?php _e('User Options', 'csm_membership'); ?></h1>
    ...
    function admin_import() {
        ...
        if ( $_SERVER["REQUEST_METHOD"] == "POST" ) {
            if ( isset( $_POST["action"] ) && $_POST["action"] == "Import Users" ) {
                ...
                if (!current_user_can('create_users') || !current_user_can('edit_users')) {
                    echo "<div class='notice notice-error is-dismissible'><p>".__( "Your account role does not have the capability to create and update users.", "csm_membership" )."</p></div>";
                } else {

                    if ( !isset( $_FILES["csv_file"] ) ) {
                        
                        echo "<div class='notice error is-dismissable'><p>".__( "You must upload a CSV file.", "csm_membership" )."</p></div>";
                        ...
                        if ( $send_welcome == true ) {
                            ...
                            echo "<p>".__( "We are currently sending welcome emails to the users that were just imported. Please don't leave this page until the below indicates the emailing is complete. This may take a long time.", "csm_membership" )."</p>";
                            echo '<iframe src="admin.php?page='.esc_attr( $_GET['page'] ).'&amp;send_emails='.absint($_POST['max_send']).'" title="Sending emails" id="sending_emails" width="400" height="300">Frames are not supported in this browser. Please click <a href="users.php?page=csm_bulk_user_management&amp;send_emails='.absint($_POST['max_send']).'" target="_blank">here</a> to send welcome emails.</iframe>';

                            return;

                        } else {

                            echo "<div class='notice notice-success is-dismissible'><p>".__("The users have been imported successfully.", "csm_membership")." ".$new_count." ".__("users were created and", "csm_membership")." ".$update_count." ".__("users were updated.", "csm_membership")."</p></div>";
            ...
            if ( isset( $_POST["action"] ) && $_POST["action"] == "Delete Users" ) {
                ...
                if (!current_user_can('delete_users')) {
                    echo "<div class='notice notice-error is-dismissible'><p>".__( "Your account role does not have the capability to delete users.", "csm_membership" )."</p></div>";
                }
                ...
                if ( isset( $_FILES["csv_file"] ) ) {
                    ...
                    echo "<div class='card'>";
                    echo "<h2 class='title'>".__("Confirm User Deletion", "csm_membership")."</h2>";
                    ...
                    if ( count( $file ) >= 1 ) {
                        ...
                        echo "<p>".__("Are you sure you want to delete the following users? If you have changed your mind for any of these users, uncheck the box:", "csm_membership")."</p>";
                        echo "<ul>";
                        ...
                        foreach ( $file as $row ) {
                            ...
                            if (user_can($cur_user, 'administrator') == false && user_can($cur_user, 'super_admin') == false) {
                                if ( $cur_user != false ) {
                        ...
                        echo "<div class='notice notice-error is-dismissible'><p>".__("The provided CSV has no valid users to delete.", "csm_membership")."</p></div>";
                    ...
                    echo "<div class='notice notice-error is-dismissible'><p>".__( "You must upload a file containing users to delete.", "csm_membership" )."</p></div>";
            ...
            if ( isset( $_POST["action"] ) && $_POST["action"] == "Confirm Delete Users" ) {
                ...
                if ( isset( $_POST["user"] ) && count( $_POST["user"] ) >= 1 ) {
                    ...
                    foreach ( $_POST["user"] as $user_id ) {
                        ...
                        if (user_can($user_id, 'administrator') == false && user_can($user_id, 'super_admin') == false) {
                    ...
                    echo "<div class='notice notice-success is-dismissible'><p>".$delete_count." ".__("users have been deleted.", "csm_membership")."</p></div>";

                } else {

                    echo "<div class='notice notice-error is-dismissible'><p>".__("No users were specified to delete.", "csm_membership")."</p></div>";
        ...
        if (current_user_can('create_users') && current_user_can('edit_users')) {
            ...
            echo "\t\t\t\t<h2 class='title'>".__("Import Users", "csm_membership")."</h2>\n";
            echo "\t\t\t\t<p><label for='csv_file'>".__( "Please select the <abbr title='Comma Separated Value'>CSV</abbr> file containing your new users using the file picker below. Existing users will be updated unless disabled below in which case they will be skipped.", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><input type='file' name='csv_file' id='csv_file' accept='.csv, .txt, text/csv, text/plain' required='required' /></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' value='".( isset( $_POST["column_username"]) ? intval( $_POST["column_username"]) : 1 )."' min='1' name='column_username' required='required' style='width: 60px;'> ".__( "Column number containing Username", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' value='".( isset( $_POST["column_email"]) ? intval( $_POST["column_email"]) : 2 )."' min='1' name='column_email' required='required' style='width: 60px;'> ".__( "Column number containing Email address", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='checkbox' name='has_headers' value='1' ".checked( 1, ( isset( $_POST["has_headers"] ) ? $_POST["has_headers"] : 1 ), false )." /> ".__( "My CSV file has headers (skips the first row)", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p>The following fields are optional. Leave blank or 0 fields that your CSV does not contain.</p>\n";
            echo "\t\t\t\t<p><label><input type='checkbox' name='skip_existing' value='1' ".checked( 1, ( isset( $_POST["skip_existing"] ) ? $_POST["skip_existing"] : 0 ), false )." /> ".__( "Disable existing user update (skips existing users)", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='checkbox' name='send_welcome' value='1' ".checked( 1, ( isset( $_POST["send_welcome"] ) ? $_POST["send_welcome"] : 0 ), false )." /> ".__( "Send WordPress new user welcome email with password", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' min='0' name='column_firstname' value='".( isset($_POST["column_firstname"] ) ? intval( $_POST["column_firstname"] ) : 0 )."' style='width: 60px;'> ".__( "Column number containing First Name", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' min='0' name='column_lastname' value='".( isset($_POST["column_lastname"] ) ? intval( $_POST["column_lastname"] ) : 0 )."' style='width: 60px;'> ".__( "Column number containing Last Name", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' min='0' name='column_website' value='".( isset($_POST["column_website"] ) ? intval( $_POST["column_website"] ) : 0 )."' style='width: 60px;'> ".__( "Column number containing Website", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' min='0' name='column_plan' value='".( isset($_POST["column_plan"]) ? intval( $_POST["column_plan"] ) : 0 )."' style='width: 60px;'> ".__( "Column number containing Role", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><select name='role'>";
            wp_dropdown_roles( ( isset($_POST["role"]) ? $_POST["role"] : get_option( "default_role" ) ) );
            echo "</select> ".__("If plan not specified by above field or invalid, assign this role.", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' value='25' min='0' name='max_send' value='".( isset($_POST["max_send"]) ? intval( $_POST["max_send"] ) : 20 )."' style='width: 60px;'> ".__("Maximum number of welcome emails to send at once", "csm_membership" )."</label></p>\n";
        ...
        if (current_user_can('export')) {
            echo "\t\t<form method='post' action='admin.php?page=".esc_attr( $_GET["page"] )."'>\n";
            echo "\t\t\t<div class='card'>\n";
            echo "\t\t\t<h2 class='title'>".__( "Export Users", "csm_membership" )."</h2>\n";
        ...
        if (current_user_can('delete_users')) {
            echo "\t\t<div class='card'>\n";
            echo "\t\t\t<form method='post' action='admin.php?page=".esc_attr( $_GET["page"] )."' enctype='multipart/form-data'>\n";
            echo "\t\t\t\t<h2 class='title'>".__("Delete Users", "csm_membership" )."</h2>\n";
            echo "\t\t\t\t<p><label for='csv_file'>".__("Please select the <abbr title='Comma Separated Value'>CSV</abbr> file containing the users to delete using the file picker below.", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><input type='file' name='csv_file' id='csv_file' accept='.csv, .txt, text/csv, text/plain' required='required' /></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' value='".( isset( $_POST["column_username"] ) ? intval( $_POST["column_username"] ) : 1 )."' min='1' name='column_username' required='required' style='width: 60px;'> ".__("Column number containing Username", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='number' size='1' value='".( isset( $_POST["column_email"] ) ? intval( $_POST["column_email"] ) : 2 )."' min='1' name='column_email' required='required' style='width: 60px;'> ".__("Column number containing Email address", "csm_membership" )."</label></p>\n";
            echo "\t\t\t\t<p><label><input type='checkbox' name='has_headers' value='1' ".checked( 1, ( isset( $_POST["has_headers"] ) ? $_POST["has_headers"] : 1 ), false )." /> ".__("My CSV file has headers (skips the first row)", "csm_membership" )."</label></p>\n";
